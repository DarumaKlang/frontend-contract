        ...
        <header className="mb-8 rounded-[30px] border border-white/80 bg-white/45 px-4 py-3 shadow-[0_20px_80px_rgba(15,23,42,0.12)] backdrop-blur-2xl sm:px-6">
          ...
          {mobileMenuOpen ? (
            <div className="mt-3 flex flex-col gap-2 rounded-[22px] border border-white/80 bg-white/70 p-3 shadow-[0_12px_45px_rgba(15,23,42,0.08)] backdrop-blur-2xl md:hidden">
              ...
              {user ? (
                <>
                  <Link href="/profile" className="rounded-2xl bg-emerald-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700" onClick={() => setMobileMenuOpen(false)}>
                    {isAdmin ? 'Admin' : 'โปรไฟล์'}
                  </Link>
                  <button type="button" onClick={() => { setMobileMenuOpen(false); logout(); }} className="rounded-2xl bg-slate-900 px-3 py-2 text-sm font-semibold text-white transition hover:bg-slate-800">
                    ออกจากระบบ
                  </button>
                </>
              ) : (
